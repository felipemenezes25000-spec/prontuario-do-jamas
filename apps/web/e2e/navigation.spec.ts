...
test.describe('Navegacao global', () => {
  ...
  test('deve exibir sidebar com itens principais do menu', async ({
  ...
  test('deve navegar entre paginas principais sem erro', async ({
    authenticatedPage: page,
  }) => {
    ...
    for (const route of routes) {
      ...
      // Each page should render an h1 without crashing
  ...
  test('deve destacar a pagina ativa no sidebar', async ({
    ...
    // Now Pacientes should be the active link
    const pacientesLink = page
      .locator('aside')
      .getByRole('link', { name: 'Pacientes' });
